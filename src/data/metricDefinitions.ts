...
export const metricDefinitions: Record<string, MetricDefinition> = {
  ...
  daily_submitted_items: {
    name: '提交数据',
    definition: '所选时间范围内用户提交的数据条数，同一数据被多次提交按提交记录累计',
  ...
  funnel_new_registered_users: {
    ...
    source: '用户注册事件',
    refresh: 'T+1',
    note: '漏斗默认展示截至所选统计期末的最新转化状态，不强制等待 30 天观察期',
  },
  ...
  approval_rate: {
    name: '通过率',
    definition: '所选时间范围内，截止对应日期招募审核通过人数/（招募审核通过+招募审核拒绝人数）',
    formula: 'approved_users / (approved_users + rejected_users)',
    source: '申请记录',
    refresh: 'T+1',
    note: '与整体通过率一致',
  },
  ...
  active_tasks: {
    name: '进行中任务数',
    definition: '截至所选统计期末状态为进行中且可被执行或提交的任务数量，按任务 ID 去重',
  ...
  daily_submissions: {
    name: '提交量',
    definition: '所选时间范围内任务提交数据条数，同一数据多次提交按提交记录累计',
    formula: 'count(submit event where event_date in range)',
    source: '分析中心',
    refresh: 'T+1',
    note: '与总提交量时间口径一致',
  },
  avg_daily_output_per_worker: {
    name: '人均日产能',
    definition: '所选统计日期的任务提交数据条数/同一统计日期有有效任务行为的去重用户数；选择周/月时建议展示区间日均',
    formula: 'daily_submissions / daily_active_workers',
    source: '分析中心',
    refresh: 'T+1',
    note: 'demo 为 38.2 条',
  },
