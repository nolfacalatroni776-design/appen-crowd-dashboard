...
export default function EditableChartCard({ id, title: defaultTitle, tooltip: defaultTooltip = '', children, className = '', showTitleTooltip = true }: any) {
  ...
  return (
    <Card className={`relative ${className} ${isEditMode ? "ring-2 ring-blue-400 ring-offset-1 transition-all" : ""} ${!config.visible ? "opacity-50 grayscale" : ""}`}>
      {isEditMode && (
        ...
        {isEditMode ? (
          <div className="space-y-2 pr-8">
            <input 
              ...
              onBlur={handleBlur}
              className="font-semibold text-sm bg-blue-50 border border-blue-200 rounded px-2 py-1 w-full focus:outline-none focus:ring-1 focus:ring-blue-500"
              placeholder="图表标题"
            />
            {showTitleTooltip && (
              <input 
